...
const TemplateREP02: React.FC = () => {
  ...
  const getFieldWidth = (field: any) => {
    ...
    if (name === 'calving_date' || name === 'fecha_paricion') return '15%';
    if (name === 'calf_caravan' || name === 'caravana_ternero') return '15%';
    if (name === 'calf_weight' || name === 'peso_nacer') return '12%';
        ...
        >
          <Box>
            <PrintHeader
              ...
              renspa={farm?.renspa || ''}
              lote={batch?.name || ''}
              title={template?.title || 'Planilla de Parición'}
          ...
          <Box sx={{ mt: 5, pt: 1.5, borderTop: '1px solid #ccc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="caption" sx={{ color: '#666', fontStyle: 'italic', fontSize: '0.65rem' }}>
              Planilla de Parición ({template?.code}) • Generado por Jhoangel AI • Sincronización offline optimizada
            </Typography>
